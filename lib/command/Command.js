...
// Timer class to allow cooldownTimer
class Timer {
    constructor(code, delay) {
        this.timer = setTimeout(code, delay);
        this.stamp = new Date();
    }
    get() {
        return ((this.timer._idleTimeout - (new Date - this.stamp)) / 1000);
    }
    clear() {
        return (this.stamp = null, clearTimeout(this.timer));
    }
}
//
    ...
    */
    ...
    cooldownCheck(msg) {
        ...
        if(this.usersOnCooldown.has(userID)) {
            ...
            if(this.restartCooldown) {
                this.cooldownTimeouts[userID].clear();
                this.cooldownTimer[userID].clear();
                this.cooldownTimeouts[userID] = new Timer(() => {
                    this.usersOnCooldown.delete(userID);
                }, this.cooldown);
                this.cooldownTimer[userID] = this.cooldownTimeouts[userID];
            }
            return false;
        }
        if(this.cooldownReturns) {
        ...
        if(this.restartCooldown) {
            ...
            this.cooldownTimer[userID] = new Timer(() => {
                this.usersOnCooldown.delete(userID);
            }, this.cooldown);
        }
        return true;
        ...
        if(args.length === 0) {
            ...
            if(this.cooldown !== 0 && !this.cooldownCheck(msg)) {
                ...
                if(this.cooldownMessage && (!this.cooldownReturns || this.cooldownAmounts[msg.author.id] <= this.cooldownReturns)) {
                    reply = typeof this.cooldownMessage === "function" ? this.cooldownMessage(msg, this.cooldownTimer) : this.cooldownMessage;
                    if(reply) {
                        msg.channel.createMessage(reply);
                    }
                }
